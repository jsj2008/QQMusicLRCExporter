...
+ (_Bool)isBackgroundOrLockScreen;
+ (_Bool)isNowCompactWidth;
+ (_Bool)isHorizonCompact;
+ (_Bool)isNarrowScreen:(int)arg1;
+ (_Bool)isCompactWidth:(int)arg1;
+ (_Bool)isUnderSplitView:(int)arg1;
+ (int)appWindowModeOf:(struct CGSize)arg1;
+ (int)getCurrentAppWindowMode;
+ (struct CGRect)getCurrentAppBounds;
+ (_Bool)isNormalPadSize;
+ (_Bool)isIPadProSize;
+ (void)initScreenConst;
+ (_Bool)isShouldSwitchWidthAndHeight;
...
+ (int)getAssetsTypeByList:(id)arg1 index:(unsigned long long)arg2;
...
+ (void)runLuaScriptContent:(id)arg1;
