//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Jan  4 2017 14:06:37).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <Foundation/NSObject.h>

@class NSMutableDictionary, VQMTADataConfig;

@interface VQMTADataConfigHolder : NSObject
{
    struct dispatch_queue_s *task_queue;
    struct sqlite3 *db;
    VQMTADataConfig *_sdkCfg;
    VQMTADataConfig *_appCfg;
    NSMutableDictionary *_appProperties;
}

+ (id)getInstance;
+ (id)initSingleton:(CDUnknownBlockType)arg1;
@property(retain, nonatomic) NSMutableDictionary *appProperties; // @synthesize appProperties=_appProperties;
@property(retain, nonatomic) VQMTADataConfig *appCfg; // @synthesize appCfg=_appCfg;
@property(retain, nonatomic) VQMTADataConfig *sdkCfg; // @synthesize sdkCfg=_sdkCfg;
- (void)updateOnlineConfig:(id)arg1;
- (void)updateConfig:(id)arg1 Type:(unsigned int)arg2;
- (id)getConfig:(unsigned int)arg1;
- (void)encode:(id)arg1;
- (void)closeDB;
- (void)deleteDB;
- (struct sqlite3 *)getDB;
- (void)setConfig:(id)arg1;
- (id)getAppProperty:(id)arg1;
- (id)initWithBlock:(CDUnknownBlockType)arg1;

@end

